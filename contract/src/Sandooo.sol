...
contract Sandooo {
    ...
    function recoverToken(address token, uint256 amount) public {
        ...
        assembly {
            ...
            case 0 {
                ...
                if iszero(call(gas(), token, 0, ptr, 68, 0, 0)) { revert(0, 0) }
            }
            case 1 { if iszero(call(gas(), caller(), amount, 0, 0, 0, 0)) { revert(0, 0) } }
        }
    }
    ...
    fallback() external payable {
        ...
        assembly {
            ...
            if iszero(eq(block_number, number())) { revert(0, 0) }

            for { let offset := 8 } lt(offset, end) {} {
                ...
                if iszero(call(gas(), tokenIn, 0, ptr, 68, 0, 0)) { revert(0, 0) }
